...
class BinomialModel(Model):
    ...
    def objective(self, coefs: NDArray) -> float:
        ...
        obj_param = -weights * (
            self._data["y"] * np.log(param) + (1 - self._data["y"]) * np.log(1 - param)
        )
    ...
    def hessian(self, coefs: NDArray) -> NDArray:
        ...
        weights = self._data["weights"] * self.trim_weights
        hess_param = weights * (
            (self._data["y"] / param**2 + (1 - self._data["y"]) / (1 - param) ** 2)
            * dparam**2
    ...
    def nll(self, params: list[NDArray]) -> NDArray:
        return -(
            self._data["y"] * np.log(params[0])
            + (1 - self._data["y"]) * np.log(1.0 - params[0])
        )

    def dnll(self, params: list[NDArray]) -> list[NDArray]:
        return [
            -(self._data["y"] / params[0] - (1 - self._data["y"]) / (1.0 - params[0]))
        ]

    def d2nll(self, params: list[NDArray]) -> list[list[NDArray]]:
        return [
            [
                self._data["y"] / params[0] ** 2
                + (1 - self._data["y"]) / (1.0 - params[0]) ** 2
            ]
        ]
