...
class NegativeBinomialModel(Model):
    ...
    def nll(self, data: dict, params: List[ndarray]) -> ndarray:
        return -(
            loggamma(params[0] + data["y"])
            - loggamma(params[0])
            + data["y"] * np.log(1 - params[1])
            + params[0] * np.log(params[1])
        )

    def dnll(self, data: dict, params: List[ndarray]) -> List[ndarray]:
        return [
            -(digamma(params[0] + data["y"]) - digamma(params[0]) + np.log(params[1])),
            data["y"] / (1 - params[1]) - params[0] / params[1],
        ]

    def d2nll(self, data: dict, params: List[ndarray]) -> List[List[ndarray]]:
        return [
            [
                polygamma(1, params[0]) - polygamma(1, params[0] + data["y"]),
                -1 / params[1],
            ],
            [
                -1 / params[1],
                params[0] / params[1] ** 2 + data["y"] / (1 - params[1]) ** 2,
            ],
        ]
