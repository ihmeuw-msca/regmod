...
class NegativeBinomialModel(Model):
    ...
    def nll(self, params: List[ndarray]) -> ndarray:
        return -(loggamma(params[0] + self._data["y"]) -
                 loggamma(params[0]) +
                 self._data["y"]*np.log(1 - params[1]) +
                 params[0]*np.log(params[1]))

    def dnll(self, params: List[ndarray]) -> List[ndarray]:
        return [-(digamma(params[0] + self._data["y"]) - digamma(params[0]) + np.log(params[1])),
                self._data["y"]/(1 - params[1]) - params[0]/params[1]]

    def d2nll(self, params: List[ndarray]) -> List[List[ndarray]]:
        return [[polygamma(1, params[0]) - polygamma(1, params[0] + self._data["y"]), -1/params[1]],
                [-1/params[1], params[0]/params[1]**2 + self._data["y"]/(1 - params[1])**2]]
